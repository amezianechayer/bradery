...
const Color:React.FC<Props> = ({setFormData,Color}) => {
    ...
    const colorArray: string[] =Color ? Color.split(',') : [];
            ...
            <div className="'mt-5">
                {selectedColors.map((selectedColors, index) => (
                    <div key={index} className="flex items-center space-x-4 mb-2">
                        ...
                        <span className="border-[1px] rounded-lg p-1 px-3 text-[14px]">{selectedColors}</span>
                        <button className="border-[1px] rounded-lg p-1 px-3 texxt-[14px]" onClick={() => handleDeleteColor(index)}>delete</button>
                    </div>
                ))}
            </div>
        </div>
